...
// 离屏更新应该永不暂停. 但是, 在一个离屏更新内部的, 暂停了的 promise 应该可以按照外部渲染的优先级进行 ping ....
// 这里的 '最早/最后 添加时间' 的翻译应该是错误的, 但是我暂时不知道什么是正确的翻译...存疑
export function markPendingPriorityLevel(
  ...
  if (earliestPendingTime === NoWork) {
    ...
    // 最早的添加时间是0, 也就是这一次就是最早的
    // 注意, 这里是把过期时间赋值给添加时间, 也就是说, 最早添加时间也就是最早添加的那个更新的过期时间
    root.earliestPendingTime = root.latestPendingTime = expirationTime;
  } else {
    // 之前有最早添加时间
    if (earliestPendingTime > expirationTime) {
      // This is the earliest pending update.
      // 如果最早添加时间大于当前过期时间, 则当前更新会更早过期, 所以当前更新的过期时间成为最早添加时间.
      root.earliestPendingTime = expirationTime;
    } else {
      const latestPendingTime = root.latestPendingTime;
      if (latestPendingTime < expirationTime) {
        // This is the latest pending update
        // 如果最后添加时间小于当前过期时间, 则当前更新的过期时间成为最后添加时间.
...
function findNextExpirationTimeToWorkOn(completedExpirationTime, root) {
  ...
  // pinged time.
  // 计算下一个应该处理的更新的过期时间, earliestPendingTime 或者 latestPingedTime (如果 earliestPendingTime === 0)
  let nextExpirationTimeToWorkOn =
    earliestPendingTime !== NoWork ? earliestPendingTime : latestPingedTime;

  // If there is no pending or pinted work, check if there's suspended work
  // that's lower priority than what we just completed.
  // 如果现在没有正在添加, 或者被ping(固定?)的工作, 检查是否有之前中断的, 且优先级比我们刚刚完成的要低的工作
  // 优先级低即意味着过期时间更长
  if (
    ...
    // 下面这段话纯属直译, 完全没明白...
    // 最低优先级的中断工作就是接下来最有可能被提交的工作.
    // 我们开始继续渲染他, 这样如果他时间到了, 也准备好提交了.
