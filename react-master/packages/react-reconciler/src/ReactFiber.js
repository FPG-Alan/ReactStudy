...
// 用于构建一个修改的 fiber (wip), 并在其上进行工作
// 比较重要的是(目前看起来), 设置了 wip 的过期时间, 而这个过期时间, 可能会跟 current 的不一样.(存疑)
export function createWorkInProgress(
  ...
  if (workInProgress === null) {
    ...
    // 我们使用了一个双缓冲池技术, 因为我们知道我们最多只需要两个版本的树结构.
    // 我们汇集了可以自由重用的"其他"节点(存疑, 不知道确切含义)
    // 这是一种懒创建, 以此来避免为永不更新的节点分配额外对象. 如果需要, 它还允许我们回收额外的内存

    // 上面应该是介绍双缓冲技术的特点, 目前还没有完全理解.

    // 这里是完全复制当前的fiber, 重建一个新的, 一样的 fiber
    ...
    // 回忆一下, 一个 fiber 的 stateNode, 指向 root, root 的 current 指向这个fiber
    ...
    // 使用 alternate 循环引用 (current 和 wip 两个 fiber)
    workInProgress.alternate = current;
    current.alternate = workInProgress;
  } else {
    // 已经存在一个 wip

    // 第一次执行的时候应该 wip 还不存在, 因此不会进入此分支
  ...
  // pendingProps 是否就是 jsx element 上的 props 呢, 这里暂时还不知道
  // 目前我走的是 reactDom.render() 路径来研究源码, 属于第一次渲染, 通过 renderRoot 进入调用的此函数 pendingProps 参数
  // 被设置为 null
  if (pendingProps !== current.pendingProps) {
    // This fiber has new props.
    // 根据这里可以推测, 如果有 props 的改变, 则参数中的过期时间应该不等于当前 fiber 的过期时间(不确定, 存疑)
    workInProgress.expirationTime = expirationTime;
  } else {
    // This fiber's props have not changed.
    // 如果 fiber 的 props 没有改变, 则 wip 的过期时间和当前 fiber 的过期时间一致
    // 然而这个过期时间应该也跟传递进来的过期时间是一致的
    // 也就是 current.expirationTime === expirationTime
