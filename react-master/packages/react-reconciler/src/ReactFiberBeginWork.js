...
// 这里的 pendingContext 暂时还不知道是什么时候加入的
// 
...
function updateHostRoot(current, workInProgress, renderExpirationTime) {
  ...
  // 第一次更新时, updateQueue 只具有firstUpdate 和 lastUpdate, 且两者一致
  ...
  // 把得到的新的状态写入 updateQueue.baseState, 并且清楚 first/last Update
  processUpdateQueue(
    workInProgress,
    updateQueue,
    nextProps,
...
function beginWork(
  ...
  // 一旦开始工作, 先清除当前WIP的过期时间(这样可以防止再次被处理?)
  workInProgress.expirationTime = NoWork;
  // 通过选择tag, 确定具体工作内容
  // 如果通过 ReactDom.render 入口进行分析, 此时这里的tag 应该是 HostRoot
  switch (workInProgress.tag) {
