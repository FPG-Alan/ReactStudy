...
function scheduleRootUpdate(
  ...
  // 将update加入 
  // 1. current(FiberNode)的updateQueue 2. current.alternate.updateQueue
  // 所谓的双缓冲
...
export function updateContainerAtExpirationTime(
  ...
  // 得到 context, 存入container, 这里的context暂时还不太清楚, 是否就是之前使用过的, 在不同组件之间传递数据的那个
  // 这里再明确一下, container 是 FiberRoot 类型, 有一个current属性
  // 指向一个Fiber对象, 而这个Fiber对象有一个stateNode属性, 指回这个 FiberRoot 对象.
