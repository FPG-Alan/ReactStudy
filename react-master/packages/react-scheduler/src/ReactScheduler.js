...
// 这就是 react 自己实现的 requestIdleCallback
// 原理是通过调度 rAF, 存储帧开始的时间, 绘制结束后调度一个 postMessage (这是一个浏览器API, 用于多窗口间信息交流)
// 在 postMessage 处理函数内做尽可能多的工作, 直到时间+帧率(到期)
// 通过将空闲调用分割成单独的事件tick, 我们确保根据可用时间计算布局, 绘画和其他浏览器工作.
// 帧速率是动态调整的
