...
function ReactRoot(container: Container, isAsync: boolean, hydrate: boolean) {
  // DOMRenderer 即为 ReactFiberReconciler
  const root = DOMRenderer.createContainer(container, isAsync, hydrate);

  // root type 为 fiberroot, fiberroot.containerInfo 即为 container
  this._internalRoot = root;
}

/*
 * 返回一个 work 实例, 似乎是 fiber 的实例
 */
...
function legacyCreateRootFromDOMContainer(
  ...
  // 首先清除所有显存内容, 关于是否水合与服务器端渲染有关, 留待后续在研究.
  // 这里如果不是SSR内容, 则进行相关操作(清除原有内容)
  if (!shouldHydrate) {
    let warned = false;
    let rootSibling;
  ...
  const isAsync = false;
  // 最终返回一个 ReactRoot 的实例, 拥有方法 { render, unmount, legacy_renderSubtreeIntoContainer, createBatch }, 拥有属性 _internalRoot:FiberRoot
...
function legacyRenderSubtreeIntoContainer(
  ...
  // 一般如果非服务器端渲染这个root是不存在的
  let root: Root = (container._reactRootContainer: any);
  // 如果不存在container._reactRootContainer, 则是第一次装载
  if (!root) {
    // Initial mount
    // 第一次装载, 给container添加 _reactRootContainer 属性, 属性值为ReactRoot
